...
import { startLevelOptions, dataNames, dataTypes, assistOptions, gamePieceOptions, climbOptions, defaultAssistOption, defaultClimbOption, defaultGamePieceOption, initiationLineOptions, rotationControl } from './dataMap'
...
export default class DataEntry extends React.Component {
	...
	render() {
		...
		autonomous.push(<Row key={20}>
			<Spacer></Spacer>
			<Row style={{ paddingBottom: 5 }}>
				<inputs.LabeledInput textStyle={styles.font.dataEntry} label={"Initiation Line Crossed"} style={dataEntryStyles.gamePieceInput}>
					<inputs.PickerInput value={this.props.data[dataNames.startLevel[0]]} options={initiationLineOptions} onValueChange={(selected) => this.dataUpdated(selected, dataNames.startLevel[0])}
						style={{
							backgroundColor:
			...
			<Row>
				<inputs.LabeledInput textStyle={styles.font.dataEntry} label={"Outer (upper) Port"} style={dataEntryStyles.gamePieceInput}>
					<inputs.ClickerInput value={this.props.data[dataNames.gameInfo.hatchesDropped]} onValueChange={(value) => this.dataUpdated(value, dataNames.gameInfo.hatchesDropped)}>
					</inputs.ClickerInput>
				</inputs.LabeledInput>
			</Row>
		...
		teleopRockets.push(<Row style={{ paddingBottom: 5 }} >
		...
		return (<View key={0} style={{ width: "100%", flex: 1, flexDirection: "column" }}>
			...
			<ScrollView>
				...
				<this.Toggle label="Can it pick up power cells from the floor?" variable={dataNames.attributes.hatchesFromFloor}></this.Toggle>
				...
				<Row>
					<inputs.LabeledInput textStyle={styles.font.dataEntry} label={"Number of power cells dropped"} style={dataEntryStyles.gamePieceInput}>
						<inputs.ClickerInput value={this.props.data[dataNames.gameInfo.hatchesDropped]} onValueChange={(value) => this.dataUpdated(value, dataNames.gameInfo.hatchesDropped)}>
						</inputs.ClickerInput>
					</inputs.LabeledInput>
				</Row>
